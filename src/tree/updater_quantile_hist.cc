...
void QuantileHistMaker::Builder::ExpandWithDepthWide(
  ...
  for (int depth = 0; depth < param_.max_depth; depth++) {
    ...
    // 1. sync histogram
    this->histred_.Allreduce(hist_[starting_index].begin,
...
void QuantileHistMaker::Builder::InitNewNode(int nid,
...

  {
    ...
    if (rabit::IsDistributed()) {
      ...
      for (int i = 0; i < gmat.cut.row_ptr.size(); i++) {
        std::cout << gmat.cut.row_ptr[i] << " ";
      }
      std::cout << "\n";
